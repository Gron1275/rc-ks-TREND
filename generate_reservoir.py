...
"""
Need to take the input domain and divide into equal partitions

then the overlap needs to be set via a parameter. If 6 grid points connect to a single reservoir, then the output of the
reservoir should also be 6 grid points that it predicts. However, if the overlap is for instance one, then the grid point directly
the first grid point should be factored into the reservoir and the grid point directly under the last grid point should be factored
in as well. So need to have a way to partition the input vector so that it includes the points above and below that are relevant.

Perhaps if the input size is 6, you slice the input vector into subarrays of size 8 with wrapping for the beginning and the end of the
array.

Once a given subarray is made, say size 8 but really meant for the 6 middle grid points that grid is evolved similar to how the reservoir
states are normally evolved. I think that each reservoir can be made in the exact same way as the erdos reservoir?
"""
def erdos_renyi_reservoir(size, degree, radius,seed):
    """TODO"""
...
def modular_reservoir(size, num_comm, global_avg_degree, mix_ratio, radius):
    ...
    threshold_mat = np.array(
        [[p_intra if (i // comm_size == j // comm_size) else p_inter for j in range(size)] for i in range(size)])
